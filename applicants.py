...
def write_newcomer_applications_to_file(team, newcomerInfos: list[NewcomerInfo]):
    ...
    for newcomer in newcomerInfos:
        ...
        f.writelines(f"* **Scheduled conflicts**: {newcomer.scheduled_conflicts}\n")
        ...
        f.writelines(f"* **Able to attend release team meetings**: {newcomer.able_to_attend_release_team_meetings}\n")
        f.writelines(f"* **Able to attend burndown meetings**: {newcomer.able_to_attend_burndown_meetings}\n")
        f.writelines(f"* **Volunteer for upcoming cycles**: {newcomer.volunteer_for_upcoming_cycles}\n")
        f.writelines(f"* **Applied previously**: {newcomer.applied_previously}\n")
...
def write_returner_applications_to_file(team, returnerInfos: list[ReturnerInfo]):
    ...
    for returner in returnerInfos:
        ...
        f.writelines(f"* **Previous Release**: {returner.previous_release_and_role}\n")
        f.writelines(f"\n**Goals**:\n{returner.goals}\n")
        f.writelines(f"\n**Contribution plans**:\n{returner.contribution_plans}\n")
        f.writelines(f"\n* **Interest in stable rooster**: {returner.interested_in_stable_roster}\n")
        f.writelines(f"* **Can volunteer for up coming cycles**: {returner.can_volunteer_for_up_coming_cycles}\n")
        i+=1
    f.close()
...
def _new_application_file(team: str, group: str) -> TextIOWrapper:
    ...
    application_file.writelines(f"# {group.capitalize()} applicant for team {team}\n")
...
def _write_applicant_header(f: TextIOWrapper, id: str, team: str, general_info: GeneralInfo):
    ...
    f.writelines(f"**Pronoun**: {general_info.pronoun}, **Slack** {general_info.slack_handle}, **GitHub** {general_info.github_handle}, **Affiliation**: {general_info.affiliation}\n")
